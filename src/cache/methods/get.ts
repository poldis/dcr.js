...
export default async function (type: String, identifier: String, { update = true, force = false, customKey = false, customWhere = false }: getOptions): Promise<any> {
	...
	if (!query || query.query.length <= 0) throw new Error("Invalid type passed to cache.get(): " + type);

	const dbRes = await this.db.query(query.query);
	if (!dbRes || dbRes.length <= 0) return null;

	if (update) await this.redis.set(`${type}_${identifier}`, JSON.stringify(dbRes[0]));
	return dbRes[0];
}